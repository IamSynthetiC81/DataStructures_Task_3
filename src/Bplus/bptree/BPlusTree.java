...
public class BPlusTree implements TucTree {
    ...
    public void setConf(BPlusConfiguration conf) {
        this.conf = conf;
    }

    public void setBPerf(BPlusTreePerformanceCounter bPerf) {
        this.bPerf = bPerf;
    }
    ...
    private void commitLookupPage() throws IOException, InvalidBTreeStateException {
        ...
        if (freeSlotPool.size() <= conf.getFirstLookupPageElements()) {
            ...
            // ResetTree the pointer and commit it
...
//    }
    ...
    public void insert(int key) {
        try{
            bPerf.insertIO(key,  String.valueOf(key), true, false);
        } catch (Exception e) {
    ...
    public boolean search(int key) {
        try{
            return bPerf.searchIO(key, true, false)[8] == 1;
        } catch (Exception e) {
            System.err.println("Error inserting key: " + key);
            e.printStackTrace();
        }
     ...
     * Validates the tree.
     *
     * TODO : Implement BPlusTree validation method
    ...
    @Override
    public Object ResetTree() {
        int pageSize = conf.getPageSize();
        int keySize = conf.getKeySize();
        int entrySize = conf.getEntrySize();

        try {
            return new BPlusTree(new BPlusConfiguration(pageSize, keySize, entrySize), new BPlusTreePerformanceCounter(true));
        } catch (Exception e) {
            System.err.println("Error resetting tree");
            e.printStackTrace();
        }
        return null;
    }
